"""DFS with a dynamically re-sorted nurse order and scarcity-aware lookahead.

Keeps the per-nurse branching of the earlier DFS solvers but re-sorts the
nurse order at every day boundary (least loaded first) and scores candidate
codes with a scarcity term that pushes surgical nurses to conserve the budget
a future B shift will need.

What it teaches: how far per-nurse branching can be pushed with better
heuristics - and where it stops.  It is clearly stronger than the plain sweep,
and still loses to group branching, because no ordering heuristic can undo a
search space that is factorially redundant to begin with.
"""
...
def solve_roster(input_file, output_file):
    ...
    # =========================================================================
    # Complete Bitmask CSP Backtracking Solver (Completeness Guarantee)
    # =========================================================================
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("usage: backtracking_dynamic_ordering.py <input_csv_path> <output_json_path>")
        sys.exit(1)
