"""Chronological DFS that decides early when a nurse must be saturated with rest.

A refinement of the plain forward-checking DFS: alongside the demand-based
value ordering it tracks, for each nurse, whether their remaining budget still
covers every day they could work.  When it does not, resting is tried *first*
rather than last, since the nurse will have to rest somewhere and doing it
early keeps their transition options open.

What it teaches: a cheap "this resource is already scarce" signal is worth
more than extra lookahead, and costs a fraction as much to compute.
"""
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("usage: backtracking_value_ordering.py <input_csv_path> <output_json_path>")
        sys.exit(1)
