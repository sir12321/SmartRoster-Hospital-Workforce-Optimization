"""Two-phase hybrid: a fast group-beam attempt, then a complete DFS fallback.

Phase 1 runs the group beam search under a short slice of the time budget,
which settles the great majority of instances immediately.  If the beam's
bounded width causes it to fail, phase 2 falls back to the per-nurse DFS with
bitmask domains and full lookahead for the remaining time.

What it teaches: the two failure modes are close to complementary.  The beam
loses on instances where its width is too narrow; the DFS loses on instances
that are large but easy.  Running the cheap one first and keeping the thorough
one in reserve gets most of both.
"""
...
# Shift bitmask constants for Phase 2
...
def solve_roster(input_file, output_file):
    ...
    # 1. Analytical Feasibility Pre-checks (Instant Fail O(N·D))
    ...
    # 2. Global structural bounds (catch instances that are only infeasible in
    #    aggregate; the per-day/per-nurse backtracking prunes below cannot see
    #    these because they only look at local or budget-only state).
    #
    #    (a) B-supply bound: H6 forces every B to be followed by R or E, so a
    #        surgical nurse can supply at most floor(K/2) B-shifts over the
    #        whole horizon (each B costs 2 toward K and consumes the next
    #        workday). If the number of S-days exceeds the total B-supply,
    #        no B-assignment can ever satisfy H7 on every S-day.
    num_s_days = future_s_days[0]
    if num_s_days > 0:
        max_b_supply = Ns * (max_shifts // 2)
        if max_b_supply < num_s_days:
            with open(output_file, 'w') as f: json.dump({}, f)
            return

    #    (b) Rest-capacity bound: H5 forces a rest day in every 6-day window,
    #        so no nurse can work more than D - floor(D/6) days regardless of
    #        their K budget. The per-day capacity prunes elsewhere only ever
    #        subtract K-budget, never mandatory rest days, so they miss
    #        instances where K is generous but the rest requirement itself
    #        caps total achievable coverage below what's needed.
    ...
    # =========================================================================
    # PHASE 1: group beam search (same scheme as beam_search.py)
    # =========================================================================
    phase1_deadline = start_time + min(T * 0.05, 1.5)
        ...
        for sset in shift_sets:
            def assign_groups_cb(idx, current_used):
                ...
                def nurse_score_cb(n, s):
                    score = cb_shifts_worked[n] * 10.0 + cb_consecutive_work[n] * 3.0
                    if n < Ns and s in ('M', 'A', 'E'):
                        rem_s = future_s_days[d + 1]
                        if rem_s > 0:
                            score += 40.0 * (rem_s / max(1, Ns))
    ...
    # =========================================================================
    # PHASE 2: Complete Bitmask CSP Backtracking Solver (Completeness Guarantee)
    # =========================================================================
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("usage: portfolio_two_phase.py <input_csv_path> <output_json_path>")
        sys.exit(1)
